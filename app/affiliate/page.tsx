...
const money = (a: number, c = 'INR') => new Intl.NumberFormat('en', { style: 'currency', currency: c }).format(a || 0);

type Summary = {
  affiliate: { id: string; type: string; status: string };
  ...
  upcoming: { booking_id: string; slot_time: string | null; status: string }[];
  referrals: { booking_id: string; status: string; amount_inr: number; created_at: string; under_review: boolean }[];
...
export default function AffiliatePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (res.status === 403) { setNotAffiliate(true); setLoading(false); return; }
      ...
      <div className={`im-legacy ${inter.className}`}>
        <div className="container" style={{ maxWidth: 480 }}>
          <div className="card" style={{ textAlign: 'center', padding: 30 }}>
            ...
            <p className="muted">
              {email ?? 'This account'} isn&rsquo;t set up as an affiliate. Affiliate accounts are
              created by an admin — reach out if you were expecting access.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="card" style={{ padding: '12px 16px', marginBottom: 18, fontSize: 13 }}>
            ...
            {summary.upcoming.map((u) => (
              <div key={u.booking_id} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0' }}>
                <span>Booking {u.booking_id.slice(0, 8)} — booked ✓</span>
            ...
            <table className="adm-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13.5 }}>
              ...
              <tbody>
                {summary.referrals.map((r) => (
                  <tr key={r.booking_id}>
                    <td style={{ padding: '9px 12px' }}>{r.booking_id.slice(0, 8)}</td>
