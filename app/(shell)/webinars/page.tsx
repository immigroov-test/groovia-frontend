...
export const metadata = { title: 'Webinars - Immigroov' };
...
export default async function WebinarsPage() {
  ...
  return <main className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
    <h1 className="text-3xl font-semibold tracking-tight text-brand-900">Upcoming webinars</h1>
    <p className="text-sm text-muted mt-2">Live guidance from people who have made the move.</p>
    <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {webinars.map((w) => <Link key={w.id} href={`/webinars/${w.slug}`} className="block">
        <Card className="h-full hover:border-brand-300 transition-colors"><CardBody className="pt-6">
          ...
          <div className="mt-2 flex items-center gap-2 text-xs text-muted"><Users className="h-4 w-4" />{w.registration_count ?? 0} / {w.capacity} registered</div>
          {w.mentor && <p className="mt-3 text-xs text-muted">Hosted by {w.mentor.display_name}</p>}
        </CardBody></Card>
      </Link>)}
    </div>
    {webinars.length === 0 && <p className="mt-10 text-sm text-muted">No webinars are scheduled yet.</p>}
  </main>;
}
