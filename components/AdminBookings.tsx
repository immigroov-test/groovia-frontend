...
export function AdminBookings() {
  ...
  const load = useCallback(async () => {
    ...
    const path = view === 'live' ? 'bookings' : 'legacy-sessions';
    try {
      if (view === 'live') setRows(null); else setPastRows(null);
      const res = await authedFetch(`/api/admin/${path}?${params.toString()}`);
      if (!res.ok) { setError('Could not load.'); return; }
      const data = await res.json();
      if (view === 'live') setRows(data); else setPastRows(data);
    } catch { setError('Could not load.'); }
  }, [authedFetch, status, q, view]);

  // Reload when the view or status filter changes (search is submitted explicitly).
                  ...
                  <tr key={s.id} className="border-b border-[--color-border] last:border-0">
                    ...
                    <td className="px-4 py-2.5 min-w-0">
                      ...
                      <span className="block text-xs text-muted">
                        {[s.duration_min ? `${s.duration_min} min` : '', money(s.amount_total, s.amount_currency)].filter(Boolean).join(' · ')}
                      </span>
                    </td>
                    <td className="px-4 py-2.5">
                      {s.status && <Badge tone={TONE[s.status] ?? 'neutral'}>{s.status.replace(/_/g, ' ')}</Badge>}
                    </td>
                  </tr>
                ))}
              </tbody>
