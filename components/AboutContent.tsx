...
export function AboutContent() {
  return <main>
    <section className="container-public py-16 sm:py-20">
      <p className="text-[13px] font-bold uppercase tracking-[0.08em] text-accent-700">About Groovia</p>
      ...
      <p className="mt-6 max-w-2xl text-lg leading-8 text-muted">Groovia brings initial guidance, lived experience, and focused live learning into one place for people planning an international move.</p>
    </section>
    ...
    <section className="container-public py-20"><div className="grid gap-12 md:grid-cols-2"><div><h2 className="font-display text-2xl sm:text-[1.75rem] font-bold text-brand-900">Our approach</h2><p className="mt-4 text-base leading-8 text-muted">Begin with the person’s goal. Make choices understandable. Show real profiles and real events only when they exist. Keep commercial details visible before commitment.</p></div><div><h2 className="font-display text-2xl sm:text-[1.75rem] font-bold text-brand-900">What we do not claim</h2><p className="mt-4 text-base leading-8 text-muted">Groovia does not promise visas, admissions, jobs, or relocation outcomes. It does not replace official requirements or regulated professional advice.</p></div></div><Link href="/how-it-works" className="mt-10 inline-flex h-12 items-center gap-2 rounded-[10px] bg-accent-600 px-6 text-[15px] font-semibold text-white shadow-[0_6px_16px_rgba(235,74,18,0.22)] hover:bg-accent-700">See how Groovia works <ArrowRight className="h-4 w-4"/></Link></section>
  </main>;
}
