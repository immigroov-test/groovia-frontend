...
// mentor profile, the mentor dashboard, and the admin mentor detail. Presentational only; the caller
// fetches the data. `bare` drops the Card wrapper so it can nest inside another card (admin detail).
export function PastSessions({ sessions, heading = 'Past sessions', bare = false }: { sessions: LegacySession[]; heading?: string; bare?: boolean }) {
  ...
  return <Card><CardBody className="pt-6">{inner}</CardBody></Card>;
}
